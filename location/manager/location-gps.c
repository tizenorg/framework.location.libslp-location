...
gps_batch_cb (gboolean enabled,
	...
	gpointer self)
{
	LOCATION_LOGD("gps_batch_cb");
	g_return_if_fail(self);
	LocationGpsPrivate* priv = GET_PRIVATE(self);
	g_return_if_fail(priv);

	if (priv->batch == NULL) {
		priv->batch = location_batch_new (num_of_location);
	}
	priv->batch->num_of_location = num_of_location;
	...
	LocationBatch **batch)
{
	...
	if (priv->batch) {
		LOCATION_LOGD("get batch from file");
		*batch = location_get_batch_file (priv->batch->num_of_location);
		ret = LOCATION_ERROR_NONE;
	} else {
...
location_gps_init (LocationGps *self)
{
	...
	priv->app_type = location_get_app_type();
	if (priv->app_type == 0) {
