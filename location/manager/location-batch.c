...
EXPORT_API gboolean
location_set_batch_details (LocationBatch *batch, char* location_info, int i)
{
...
location_get_batch_file (int num_of_location)
{
	...
	LocationBatch* batch_data = location_batch_new(num_of_location);
	batch_data->num_of_location = num_of_location;
	...
	if (fd != NULL) {
		...
		for (i = 0; i < num_of_location; i++) {
			if (fgets(buf, BATCH_SENTENCE_SIZE, fd) != NULL) {
				location_set_batch_details(batch_data, buf, i);
			}
